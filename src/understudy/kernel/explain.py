...
if TYPE_CHECKING:
    ...
    class _ExplainerFunc(Protocol):
        ...
        ) -> tuple[dict[str, Any], dict[str, Any], list[str], str, str]: ...
...
def _safe_eval_model(
    ...
    try:
        ...
        if hasattr(val, "as_long"):
            return val.as_long()
        return fallback_val
    except (z3.Z3Exception, AttributeError, ValueError, RuntimeError):
        return fallback_val
...
# --- Modular Invariant Explainer Handlers ---
...
def _explain_k1(
    *,
    ...
    for svc in sorted(workloads):
        ...
        min_reps = _safe_eval_model(model, ctx.constants.get(f"min_replicas[{svc}]"), min_reps_raw)
    ...
    return failing_facts, counterexample, unsat_core, human_reason, guidance
...
def _explain_k3(
    *,
    ...
    if target_commit != "unknown" and ctx.has_fact(f"target_contains_migration[{target_commit}]"):
...
def _explain_k9(
    *,
    ...
    inv_list = list(raw_inv) if isinstance(raw_inv, (list, tuple, set, frozenset)) else [raw_inv]
...
def _explain_default(
    *,
    ...
    counterexample = {"invariant_id": inv_id, "statement": statement}
    failing_facts: dict[str, Any] = {}
    guidance = f"Review invariant {inv_id} constraint specification."
    return failing_facts, counterexample, unsat_core, human_reason, guidance


_EXPLAINERS: dict[str, _ExplainerFunc] = {
    "K1": _explain_k1,
    "K2": _explain_k2,
    "K3": _explain_k3,
    "K4": _explain_k4,
...
def explain_veto(
    ...
    explainer = _EXPLAINERS.get(inv_id, _explain_default)
    failing_facts, counterexample, unsat_core, human_reason, guidance = explainer(
        inv_id=inv_id,
        statement=statement,
        plan=plan,
        ctx=ctx,
